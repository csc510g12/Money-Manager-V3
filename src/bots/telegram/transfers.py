...
async def transfer_start(
    ...
    if response.status_code == 200:
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    account["name"], callback_data=f"source_{account['_id']}"
                )
            ]
        ...
        await update.message.reply_text(
            "Select the source account:", reply_markup=reply_markup
        )
...
async def select_source(
    ...
    if response.status_code == 200:
        ...
        dest_accounts = [
            acct for acct in accounts if acct["_id"] != source_account_id
        ]
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    account["name"], callback_data=f"dest_{account['_id']}"
                )
            ]
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.message.edit_text(
            "Select the destination account:", reply_markup=reply_markup
        )
...
async def send_confirmed_transfer(
    ...
    if query.data == "confirm_yes":
        ...
        response = requests.post(
            f"{TELEGRAM_BOT_API_BASE_URL}/accounts/transfer",
            json=payload,
            headers=headers,
        )
        if response.status_code == 200:

...
class TestAccountEdgeCases:
    async def test_create_account_extremely_high_balance(self, async_client_auth: AsyncClient):
        """
    ...
    async def test_update_account_with_no_changes(self, async_client_auth: AsyncClient):
        """
        ...
        response = await async_client_auth.put(f"/accounts/{account_id}", json={})
        # Expect a 400/422 error indicating no fields were provided.
        assert response.status_code in (400, 422), response.json()

    async def test_update_account_with_extra_fields(self, async_client_auth: AsyncClient):
        """
        ...
        create_resp = await async_client_auth.post(
            "/accounts/",
            json={"name": "Extra Fields", "balance": 1000.0, "currency": "USD"}
        )
        account_id = create_resp.json()["account_id"]

        # Send an update including an extra field.
        update_payload = {"balance": 1500.0, "currency": "EUR", "extra": "value"}
        response = await async_client_auth.put(f"/accounts/{account_id}", json=update_payload)
        # Expect a successful update ignoring the extra field.
        assert response.status_code == 200, response.json()

    async def test_delete_account_affects_listing(self, async_client_auth: AsyncClient):
        """
        ...
        create_resp = await async_client_auth.post(
            "/accounts/",
            json={"name": "To Be Deleted", "balance": 500.0, "currency": "USD"}
        )
        account_id = create_resp.json()["account_id"]

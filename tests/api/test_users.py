...
@pytest.mark.anyio
class TestTransferToUser:
    async def test_successful_transfer(self, async_client: AsyncClient):
        response = await async_client.post(
            "/transfer-to-user",
            json={
                "destination_account": "recipient_user_id",
                "amount": 50
            },
            headers={"token": "valid_user_token"}
        )
        assert response.status_code == 200
        assert response.json()["message"] == "Transfer successful"

    async def test_transfer_negative_amount(self, async_client: AsyncClient):
        response = await async_client.post(
            "/transfer-to-user",
            json={
                "destination_account": "recipient_user_id",
    ...
    async def test_transfer_zero_amount(self, async_client: AsyncClient):
        response = await async_client.post(
            "/transfer-to-user",
            json={
                "destination_account": "recipient_user_id",
    ...
    async def test_insufficient_funds(self, async_client: AsyncClient):
        response = await async_client.post(
            "/transfer-to-user",
            json={
                "destination_account": "recipient_user_id",
                "amount": 1000000  # Exceeds balance
            },
            headers={"token": "valid_user_token"}
        )
        assert response.status_code == 400
    ...
    async def test_source_account_not_found(self, async_client: AsyncClient):
        response = await async_client.post(
            "/transfer-to-user",
            json={
                "destination_account": "recipient_user_id",
            ...
            headers={"token": "user_with_no_checking_account"}
        )
        assert response.status_code == 404
        assert response.json()["detail"] == "Source account not found"

    async def test_recipient_user_not_found(self, async_client: AsyncClient):
        response = await async_client.post(
            "/transfer-to-user",
            json={
                "destination_account": "nonexistent_user_id",
                "amount": 50
            },
            headers={"token": "valid_user_token"}
        )
        assert response.status_code == 404
        assert response.json()["detail"] == "Recipient user not found"

    async def test_recipient_account_not_found(self, async_client: AsyncClient):
        response = await async_client.post(
            "/transfer-to-user",
            json={
                "destination_account": "recipient_user_with_no_checking_account",
                "amount": 50
            },
            headers={"token": "valid_user_token"}
        )
        assert response.status_code == 404
        assert response.json()["detail"] == "Recipient checking account not found"
